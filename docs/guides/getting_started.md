...
module "clumio_protect_gcp" {
  ...
  project_id                = data.google_project.current.project_id
  clumio_control_plane_id   = clumio_gcp_connection.gcp_connection.clumio_control_plane_id
  clumio_control_plane_role = clumio_gcp_connection.gcp_connection.clumio_control_plane_role
  is_gcs_enabled            = true
}
```
...
configured with credentials whose principal can create Workload Identity Pools, service accounts, and
IAM bindings in the target project. For other ways to configure the Google provider, see:
