...
If connecting a GCP project, the Google provider authenticates using Application Default Credentials.
Log in before running Terraform:

```shell
$ gcloud auth application-default login
```
Information on other ways to configure the Google provider can be found here:
...
Terraform configuration with the Google provider and the Clumio GCP module.
...
resources will be provisioned. When ready, run `terraform apply`.
