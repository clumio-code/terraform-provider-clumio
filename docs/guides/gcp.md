...
- [Bring Your Own Service Account](#byo-sa)
...
By default, the Clumio GCP module creates a customer-side service account in your project for the
Clumio service account to impersonate. To use a service account you already manage instead, set the
`customer_service_account_email` input to its email address — the module then skips creating one and
wires your existing service account into the required IAM bindings. The rest of the configuration is
identical to the [Basic](#basic) example.
